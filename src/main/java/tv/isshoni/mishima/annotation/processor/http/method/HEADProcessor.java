...
public class HEADProcessor extends SimpleHTTPMethodProcessor<HEAD> {
    ...
    public HEADProcessor(@Inject HTTPService service, @Inject IWinryContext context) {
        super(service, context, HEAD.class);
    }

    @Override
    protected RuntimeException validate(IAnnotatedMethod method, Object target, HEAD annotation) {
        if (!this.service.hasHandler(HTTPMethod.GET, getPath(annotation))) {
